...
import { FONT_FACES_CSS } from '../fonts';
...
/**
 * Inject Manrope + Inter @font-face declarations from the bundled fonts.ts.
 *
 * Previously these came from fonts.googleapis.com at runtime, which caused
 * FOUT (flash of unstyled text) on every dashboard load and broke entirely
 * on air-gapped HA instances. The fonts are now embedded as base64 woff2
 * data URIs in src/fonts.ts (generated by scripts/inline-fonts.mjs from
 * @fontsource/{manrope,inter}).
 *
 * Material Symbols was previously CDN-loaded too; verified unused (every
 * icon in src/ is mdi:* served by HA's native <ha-icon>), so it's dropped.
 *
 * Idempotent: checks for our marker <style> element and skips if found.
 */
const FONT_STYLE_ID = 'lumina-cards-fonts';
